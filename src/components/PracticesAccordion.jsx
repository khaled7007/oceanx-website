...
function AccordionItem({ practice, isOpen, onToggle, isEn, discoverMore }) {
  const title = isEn ? practice.titleEn || practice.title : practice.title
  const subtitle = isEn ? practice.subtitleEn || practice.subtitle : practice.subtitle
  return (
    <div className="border-b border-gray-100">
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between gap-3 py-5 sm:py-6 min-h-[56px] text-start group touch-manipulation"
      >
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="pb-6 sm:pb-8 pe-2 sm:pe-11 ps-1">
    ...
    <section className="py-16 sm:py-24 lg:py-32 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 sm:mb-12 pb-6 border-b border-gray-100">
          <div>
            <span className="section-label block mb-3">{t('practicesAccordion.label')}</span>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">
              {t('practicesAccordion.title')}
              <span className="text-brand-blue">{t('practicesAccordion.titleAccent')}</span>
            </h2>
          </div>
          <Link
            to="/about"
          ...
          >
            {t('practicesAccordion.aboutLink')}
        ...
        <div>
          {PRACTICES.map((p) => (
            <AccordionItem
              ...
              isEn={isEn}
              discoverMore={t('practicesAccordion.discoverMore')}
            />
          ))}
        </div>
      </div>
